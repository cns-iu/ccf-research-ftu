...
Transfer learning was utilized by starting the Faster R-CNN model with [COCO dataset weights](add link to file). The [pets dataset configuration file](add link to file), which includes the frozen inference graph for transfer learning, and ["labelmap.pbtxt" file](add link), which contains the custom label map for glomeruli, were used to begin training the model.
...
![config file screenshot](add link)
...
Once the training loss was acceptable, the process was stopped using "Ctrl+C". In our experience, this took 2425 epochs. From the same window, the resulting inference graph was exported using the ["export_inference_graph.py" script](add link): 
...
The ["object_detection_Glomeruli.ipynb" Jupyter notebook](add link) was utilized for making predictions with the Faster R-CNN Glomeruli Detection model, as well as calculating the accuracy metric Intersect over Union (IoU).
